CREATE DATABASE T35_TranThuHien;
GO
USE T35_TranThuHien;
GO
...
exec sp_addrolemember nhom1,tranthuhien2
...
CREATE INDEX IX_SoTien_ThuChi ON ThuChi(SoTien) WITH FILLFACTOR = 80--hệ số điền đầy tạo ra khoảng trống để các bản ghi được chèn vào đó
...
-- tạo cho cô 1 index trên 2 cộ họ tên và ngày chi
CREATE INDEX Hoten on SinhVien(ChiTieu) 

--chỉ mục toàn văn
--tăng tốc độ truy vấn với dữ liệu dạng chuỗi
--nhóm chứa các chỉ mục full text (tìm kiếm 1 chuỗi văn bản)
...
go --không thể insert, update, delete trên view được tạo từ 2 bảng trở lên
SELECT*FROM ThuChiSinhVien --delete từ trong ra, insert từ ngoài vào
...
SELECT top(1) lc.TenLoai AS LoaiChiTieu, SUM(c.SoTien) AS TongChiTieu-- hiện ra loại chi tiêu nhỏ nhất
FROM ChiTieu c
JOIN LoaiChiTieu lc ON c.MaLoaiChiTieu = lc.MaLoaiChiTieu
GROUP BY lc.TenLoai
...
JOIN LoaiChiTieu lc ON c.MaLoaiChiTieu = lc.MaLoaiChiTieu
GROUP BY lc.TenLoai
HAVING SUM(c.SoTien) > 500000 --having thì khác where cho biểu thức vào được
...
create PROCEDURE CapNhatSinhVien
    @MaSV char(10),-- tăng tốc độ truy vấn và tái sử dụng mã 
    @HoTen NVARCHAR(255),
    @Email NVARCHAR(255),
    @DienThoai CHAR(20),
    @DiaChi NVARCHAR(255)
...
END
go
EXEC CapNhatSinhVien 'SV01', N'Nguyễn Văn Hiếu', N'nguyenvanhieu@gmail.com', '0123456789', N'Hà Nam'
select*from SinhVien
...
END
go
EXEC ChenChiTieu 'MCT012', 'SV01', 'LCT01', 500000.0, N'Mua sách mới', '2023-01-10'
go
select*from ChiTieu
go
...
END
go
TimKiemChiTieuTheoNgay '2023-01-15'
go
...
----------------------------------------------------
--PHẦN 7
-- Trigger after(for) insert-- chạy xong lệnh thì mới kích hoạt trigger
...
go
INSERT INTO SinhVien (MaSV, HoTen, Email, DienThoai, DiaChi) 
VALUES('SV014', N'Nguyễn Văn Hậu', N'nguyenvanhau@gmail. com', '0123456789', null)
select * from SinhVien
go
...
select * from LoaiChiTieu
go
--4. Trigger INSTEAD OF UPDATE --trigger instead of chạy trước câu lệnh 
...
BEGIN
	...
	begin
		print N'Mã Thu Chi tồn tại.'-- sửa in ra mã thu chi và số tiền
		update ThuChi
		set SoTien = (select SoTien from deleted)
		where MaThuChi = (select MaThuChi from deleted)
		select * from inserted -- sau khi cập nhâpk
		select * from deleted
	end
	else
	begin
